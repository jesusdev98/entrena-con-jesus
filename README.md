...
Use `npm run build`, rather than bare `ng build`, to include dependency notices, generate the expected-resource manifest and finalize service-worker hashes. PWA plumbing prefetches current lazy chunks and local assets. Settings waits for an activated controlling worker and every required cached resource, within a bounded preparation window. Readiness checks pass at root and subpath in Chromium and WebKit; Chromium also completes offline reload/edit workflows. **WebKit offline reload remains unresolved:** both paths produce `page.reload: WebKit encountered an internal error` after disconnecting. Offline reload on iOS is not guaranteed; real-device installation remains unverified. Development mode does not enable the service worker.
